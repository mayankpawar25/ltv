...
<?php 
   ...
   $c = $rec->comments()->paginate(TICKET_THREAD_PAGE_LENGTH) ;
...
<div id="thread_{{ $comment->id }}" class="{{ ($comment->user_type == 1) ? 'thread-member' : 'thread-customer' }}" style="padding: 20px; border: 1px solid #dce1ef; border-radius: 4px; font-size: 14px; margin-bottom: 40px;">
   <div class="row">
      <div class="col-md-3">
         <?php $commenter_info = $comment->get_commenter_information(); ?>
         <p><?php echo $commenter_info['name'] ?></p>
         <div><?php echo $commenter_info['type'] ?></div>
         <br>
         <div><a class="btn btn-sm btn-success" href="{{ route('convert_ticket_to_task', $comment->id) }}">@lang('form.covert_to_task')</a></div>
      </div>
...
@endforeach
{{ $c->links('admin.crm.layouts.pagination_bootstrap-4') }}
@endif