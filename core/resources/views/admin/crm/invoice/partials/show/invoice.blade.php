            ...
            <table class="table items estimate-items-preview">
               ...
               <tbody class="ui-sortable">
                  ...
                  <tr>
                     ...
                     <td class="text-right">{{-- format_currency($row->rate) --}}{{ $row->rate }}</td>
                     <td class="text-right">{{ display_tax_rate_in_item_list($row->tax_id , $rec) }}</td>
                     <td class="amount text-right" >{{-- format_currency($row->sub_total) --}}{{ $row->sub_total }}</td>
                  </tr>
                  @endforeach
                  @endif
               </tbody>
         ...
         <table class="table text-right">
            <tbody>
               <tr>
                  ...
                  <td class="subtotal">{{-- format_currency($rec->sub_total) --}}{{ $rec->sub_total }}</td>
               </tr>
               @if(isset($rec->discount_total) && ($rec->discount_total > 0))
               <tr>
                  <td><span class="bold">@lang('form.discount')</span>
                  </td>
                  <td class="subtotal">{{-- format_currency($rec->discount_total) --}}{{ $rec->discount_total }}</td>
               </tr>
               @endif
               ...
               <tr class="tax-area">
                  ...
                  <td>{{-- format_currency($t->amount) --}}{{ $t->amount }}</td>
               </tr>
               @endforeach
               @endif
               ...
               <tr>
                  ...
                  <td>{{-- format_currency($rec->adjustment, true, $rec->get_currency_symbol()) --}}{{ $rec->adjustment }}</td>
               </tr>
               @endif
               <tr>
                  <td><span class="bold">@lang('form.total')</span></td>
                  <td class="total">{{-- format_currency($rec->total, true, $rec->get_currency_symbol()) --}}{{ $rec->total }}</td>
               </tr>
               <tr>
                  <td><span class="bold">@lang('form.total_paid')</span></td>
                  <td><span class="">{{-- format_currency($rec->amount_paid, true, $rec->get_currency_symbol()) --}}{{ $rec->amount_paid }}</span></td>
               </tr>
               @if(isset($rec->applied_credits) && $rec->applied_credits > 0)
               <tr>
                  <td class="bold">@lang('form.applied_credits')</td>
                  <td>{{-- format_currency($rec->applied_credits, true, $rec->get_currency_symbol()) --}}{{$rec->applied_credits }}</td>
               </tr>
               @endif
               ...
               <tr>
                  ...
                  <td><span class="{{ ($amount_due > 0) ? 'text-danger' : '' }}">{{-- format_currency($amount_due, true, $rec->get_currency_symbol()) --}}{{ $amount_due }}</span></td>
               </tr>
            </tbody>
         </table>
      </div>
         ...
         <table class="table" width="100%" id="data">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td class="text-right">{{-- format_currency($payment->amount, TRUE, $rec->get_currency_symbol()) --}}{{ $payment->amount }}</td>
               </tr>
               @endforeach
            </tbody>
         </table>
