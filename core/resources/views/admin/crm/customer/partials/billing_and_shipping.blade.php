   ...
   <div class="col-md-6">
      <div style="padding: 10px;">
         ...
         <div class="form-group">
            ...
            <textarea id="shipping_address" name="shipping_address" placeholder="{{ __('form.street') }}" class="form-control " {{ ($disable) ? 'disabled' : '' }}>{{ old_set('shipping_address', NULL, $rec) }}</textarea>
            <div class="invalid-feedback">@php if($errors->has('shipping_address')) { echo $errors->first('shipping_address') ; } @endphp</div>
         </div>
         <div class="form-row">
            <div class="form-group col-md-6">
               <input type="text" class="form-control " placeholder="{{ __('form.city') }}" name="shipping_city" value="{{ old_set('shipping_city', NULL, $rec) }}" {{ ($disable) ? 'disabled' : '' }}>
               <div class="invalid-feedback">@php if($errors->has('shipping_city')) { echo $errors->first('shipping_city') ; } @endphp</div>
            </div>
            <div class="form-group col-md-6">
               <input type="text" class="form-control " placeholder="{{ __('form.state') }}" name="shipping_state" value="{{ old_set('shipping_state', NULL, $rec) }}" {{ ($disable) ? 'disabled' : '' }}>
               <div class="invalid-feedback">@php if($errors->has('shipping_state')) { echo $errors->first('shipping_state') ; } @endphp</div>
            </div>
         </div>
         <div class="form-row">
            <div class="form-group col-md-6">
               <input type="text" class="form-control " placeholder="{{ __('form.zip_code') }}" name="shipping_zip_code" value="{{ old_set('shipping_zip_code', NULL, $rec) }}" {{ ($disable) ? 'disabled' : '' }}>
            ...
            <div class="form-group col-md-6">
               <div class="select2-wrapper">
                  ...
                  <?php echo form_dropdown("shipping_country_id", $data['country_id_list'], old_set("shipping_country_id", NULL, $rec), "class='form-control selectpicker'  {{ $shipping_country_disable }}") ?>
               </div>
