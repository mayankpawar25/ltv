...
class LoginController extends Controller
{
  ...
  */
  public function findOrCreateUser($user, $provider){
    ...
    $authUser = new User;
    $authUser->first_name = ($user->getName() != null)?$user->getName():$user->user['first_name'];
    $authUser->last_name = (isset($user->user['last_name']))?$user->user['last_name']:$user->user['family_name'];
    $authUser->date_of_birth = (isset($user->user['birthday']))?$user->user['last_name']:'';
    $authUser->username = $provider . '_' . $user->id;
    $authUser->email = ($user->getEmail() != null)?$user->getEmail():$user->id . '@' . $provider;
    $authUser->email_verified = ($user->user['verified_email']==true)?'1':'0';
    $authUser->gender = (isset($user->user['gender']))?(ucwords($user->user['gender'])):'';
    $authUser->sms_ver_code = 1234;
