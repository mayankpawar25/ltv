...
class PaymentController extends Controller
{
    ...
    {
        return view('admin.crm.payment.index');
    }
        ...
        if (count($data) > 0)
        {
            foreach ($data as $key => $row)
            {
                ...
                $rec[] = array(
                    ...
                    $row->transaction_id,
                    // json_encode($row->invoice->related_to),
                    (isset($row->invoice->related_to))?($row->invoice->related_to->name)?$row->invoice->related_to->name:$row->invoice->related_to->first_name.' '.$row->invoice->related_to->last_name:'',
    ...
    {       
        ...
        $data = [];
       return view('admin.crm.payment.show', compact('data'))->with('rec', $payment);
    }
    ...
    {   
        ...
        $payment->date = sql2date($payment->date);
       return view('admin.crm.payment.create', compact('data'))->with('rec', $payment);
    }
    ...
    function download_receipt_pdf(Payment $payment)
    {
        ...
        if ($payment)
        {
            if($payment)
            {
                ...
                $data['html']       = view('admin.crm.payment.print')->with('rec', $payment)->render();
