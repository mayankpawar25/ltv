...
{
    use HasApiTokens; use Notifiable;  use SoftDeletes;
   ...
   protected $fillable = [
       ...
       'notes', 'default_language', 'currency_id', 'created_by','assigned_to'
    ];
    ...
    function groups()
    {
        return $this->belongsToMany(CustomerGroup::class ,'tag_customers_groups','user_id','group_id');
    }
    ...
    public static function statistics()
    {
        ...
        $customer = User::select( DB::raw('IFNULL(inactive, 0) as inactive'), DB::raw('count(*) as total'))
            ->groupBy('inactive')
    ...
    {   
        ...
        // $data['assigned_to_list'] = array('' => $select) +  Models\StaffUser::orderBy('first_name','ASC')->pluck('first_name','id')->toArray();;
